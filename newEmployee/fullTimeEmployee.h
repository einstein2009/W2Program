...
public:
	void set(string first, string last, long id, double salary, double bonus);
		//Function to set the first name, last name, id, and salary according to the parameters.
		//Postcondition: firstName = first; lastName = last; personId = id; empSalary = salary; empBonus = bonus;
	...
	fullTimeEmployee(string first = "", string last = "", long id = 0, double salary = 0, double bonus = 0);
		...
		//Sets the first name, last name, id, salary, and bonus according to the parameters. If no value is specified, the default values are assumed.
		//Postcondition: firstName = first; lastName = last; personId = id; empSalary = salary; empBonus = bonus;
private:
	double empSalary;
	double empBonus;
